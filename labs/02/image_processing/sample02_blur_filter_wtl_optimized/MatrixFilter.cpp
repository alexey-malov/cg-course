...
std::unique_ptr<Gdiplus::Bitmap> CMatrixFilter::ApplyFilter(Gdiplus::Bitmap& srcBitmap)
{
	...
	for (int y0 = 0; y0 < h; ++y0)
	{
		...
		const DWORD* pSrcLine = reinterpret_cast<const DWORD*>(pSrcData + (srcData.Stride * y0));

		// Вычисляем адрес начала строки с индексом y0 в результирующем изображении
		DWORD* pDstLine = reinterpret_cast<DWORD*>(pDstData + (dstData.Stride * y0));
		...
		for (int x0 = 0; x0 < w; ++x0)
		{
			...
			for (size_t row = 0; row < m_matrixHeight; ++row, ++y)
			{
				...
				const DWORD* pSrcFilterLine = reinterpret_cast<const DWORD*>(pSrcData + (srcData.Stride * srcY));
